...
$mhcurl->config['cookie'] = 'cookieqhhcomict';

$imgcurl = new CurlModel();
$imgcurl->config['cookie'] = 'cookieqimgt';
    ...
    foreach($pages as $kp => $pval){
     ...
     if(!$pid || $pinfo['isimg']!= 1){
      ...
      if( 1 == $r['flag']){
       ...
       $pagedata['img'] = '';
       $pagedata['ourl'] = $uploadData['imgurl'];
       echo ("\n+++ Error cid:$comicid Vid:$vid Pid:$pid Ourl: $uploadData[imgurl] ++++\n");
      }
      $pagedata['rtime'] = time();
      if(!$pid){
      ...
      echo "\n+++ Add cid:$comicid Vid:$vid Pid:$pid ImgUrl: $pagedata[img] ++++\n";
      sleep(5);
