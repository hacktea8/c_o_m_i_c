...
function bookMark(){
	...
	if(ref && !/(hacktea8|dm456|hao123|4399|qq|)/gi.test(ref.toLowerCase())){
		if(confirm('爱漫画，让你爱上漫画！收藏爱漫画，方便浏览最新漫画！')){
			window.external.addFavorite('http://www.hacktea8.com/?Fav', '爱漫画,在线漫画,为您提供最新、最好看的漫画');
		}else{
		}
	}
...
function hotComic(){
	...
	for(var i=0;i<arrLink.length;i++){
		document.write("<a href=\"/comic/" + arrLink[i][0] + "\" alt=\"" + arrLink[i][1] + "\">" + arrLink[i][1] + "</a> ");
	}
}

function otherNav(){
	document.write('<li class="other"><a href="http://www.dm456.com/" style="color:yellow;" target="_blank"><strong>日本动漫</strong></a></li>');
	document.write('<li class="other"><a href="http://www.dm456.com/" style="color:yellow;" target="_blank"><strong>动画片</strong></a></li>');
	$('kw').value='请输入搜索内容';
}
...
function initGame(){
	var gamepage = $('webgamepage');
	if(gamepage.src=='about:blank'){
		gamepage.src = '/v2/money/webgame.html';
	}
}