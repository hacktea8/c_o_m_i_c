...
function bookMark(){
 window.setTimeout(addBookmarkDelay,50);
}
function addBookmarkDelay(){
 if (document.all){  
  try{  
   window.external.addFavorite(window.location.href,site_name+"网页版");  
  }catch(e){  
   alert( "对不起！添加到收藏夹失败，请使用Ctrl+D进行添加！" );  
  }  
 }else if (window.sidebar){  
  window.sidebar.addPanel(site_name+"网页版", window.location.href, "");  
 }else{  
  alert( "对不起！添加到收藏夹失败，请使用Ctrl+D进行添加！" );  
 }  
}
function errimg(obj){
 obj.src = _cdn_url+'/public/images/comic_404.png';
 return true;
}
function toFav(){
